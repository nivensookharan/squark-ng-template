...
- **Design tokens (colour palette, radius, shadow, spacing, typography, z-index) are defined in [`../../ui-design/references/design-tokens.md`](../../ui-design/references/design-tokens.md)**. Copy the `Reference styles.scss` block from that file into the project's `src/styles.scss` — never hard-code colour, radius, or shadow values in components.
